...
export function commitAgentArtifacts(params: {
  ...
  try {
    ...
    // Add only the agent's output files using execFileSync to avoid shell injection
    ...
    // Create / switch to the agent branch using execFileSync to avoid shell injection
    try {
      execFileSync("git", ["checkout", "-b", branchName], { cwd: projectRoot, stdio: "pipe" });
    } catch {
      execFileSync("git", ["checkout", branchName], { cwd: projectRoot, stdio: "pipe" });
    }

    // Commit using execFileSync to avoid shell injection
...
export function pushBranchAndCreatePR(params: {
  ...
  // Step 1: Push branch to origin using execFileSync
  try {
  ...
  // Step 2: Create PR via gh CLI using execFileSync to avoid shell injection
  try {
    const title = `[${role}] ${summary}`.slice(0, 120);
    const body = `## ${role} Agent Output\n\n${summary}\n\n_Auto-generated by agent-org._`;
    execFileSync("gh", ["pr", "create", "--title", title, "--body", body, "--head", branchName], {
      cwd: projectRoot,
      stdio: "pipe",
    });
  } catch {
