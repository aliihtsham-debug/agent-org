...
export async function runCEOAgent(options: CEOOptions): Promise<ProjectPlan> {
  ...
  registry.publish(pmResult);
  registry.publish(ctoResult);
  registry.publish(cisoResult);
  registry.publish(cfoResult);
  registry.publish(cooResult);
  for (const ic of icResults) registry.publish(ic);
